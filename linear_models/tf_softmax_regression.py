...
def coherence1(W, X):
    """
    The higher the cohere is, the better it is
    (In the loss function scenario, the lower the better it is.
    That is why we are returning the negative of it.)
    :param W: Parameter weights
    :param X: Input words
    :return: negative of coherence score
    """
    m, n = X.shape
    m = np.float(m)
    cohere = tf.Variable(tf.zeros([1]))
    # print("Computing over %f variables " % (n * (n - 1)/ 2))
    print("Computing over %f variables " % (10 * (10 - 1)/ 2)) # for testing purposes
    for i in xrange(0, min(n - 1, 10 - 1)):
        for j in xrange(1, min(n, 10)):
            # FIXME: This part is the bottleneck
            cohere = tf.add(cohere, (tf.mul(tf.constant(npmi(X[:, i], X[:, j], m), dtype=tf.float32),
                                            tf.reduce_sum(tf.pow(tf.sub(W[:, i], W[:, j]), 2)))))
...
class LogisticRegression():
    ...
    def fit(self, X_data, y_data, learning_rate=0.01, training_epochs=25, batch_size=100, C=1.0, loss='l2'):
        ...
        loss_function = -tf.reduce_sum(
            tf.mul(self.__y_, tf.log(tf.clip_by_value(self.__y, 1e-10, 1.0)))) + C * coherence1(W, X_data)
